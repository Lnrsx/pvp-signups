...
class Booking(object):
    ...
    type: Optional[`str`]
        The type of the boost. Can be 'Hourly', '1 win' or 'Set rating'. Could be None.
    ...
    rating: Optional[`str`]
        The rating of the boost. Can be in format `int` or `int-int`. Could be None.
    price_recommendation: Optional[`int`]
    ...
    async def validate(cls):
        ...
        del sheet[0]
        cachefields = [[
            statuses[b.status], b.id, b.gold_realms or "N/A", b.booster, b.boost_cut,
            b.booster_2, b.boost_cut_2, str(b.author.id), str(b.ad_cut),
            str(b.price), str(b.author), 'Pending Booking Completion'] for b in cls.instances]
        not_on_sheet = [x for x in cachefields if x not in sheet]
        not_in_cache = [x for x in sheet if x not in cachefields]
    ...
    def author(self) -> discord.User:
        ...
        .. note::
        If the booking was loaded for the cache on startup, the author but be retrieved from ID.
        """
    ...
    async def compile(self):
        ...
        await self._get_spec(self.buyer_class)
        await self._get_rating_range(self.type)
    ...
    async def _get_name_faction_class(self):
        ...
        if cfg.settings["auto_faction_class_input"]:
            ...
            if response['status'] == 200:
                ...
                self.buyer_name = buyer_name.capitalize()
                self.buyer_realm = buyer_realm.capitalize()
    ...
    async def manual_class_input(self):
        ...
        buyer_class = await react_message(
            ...
            'or react with ❌ to cancel the booking', '❌')
        if buyer_class.capitalize() in dictionaries.spec_emotes.keys():
            self.buyer_name = self.buyer_name.capitalize()
            self.buyer_realm = self.buyer_realm.capitalize()
            self.faction = self.faction.capitalize()
            self.buyer_class = buyer_class.capitalize()
        else:
            await self.author.send('Class not recognised, please try again.')
            await self.manual_class_input()

    async def _get_spec(self, buyer_class):
        ...
        if buyer_spec.capitalize() not in dictionaries.spec_emotes[buyer_class].keys() and buyer_spec.capitalize() not in list(dictionaries.class_specs_abbreviations[buyer_class].keys()):
            await self.author.send('Spec not recognised, please try again.')
            await self._get_spec(buyer_class)

        else:
            # translates spec abbreivation to proper class name if it is used
            if buyer_spec.capitalize() in list(dictionaries.class_specs_abbreviations[buyer_class].keys()):
                buyer_spec = dictionaries.class_specs_abbreviations[buyer_class][buyer_spec.capitalize()]

            self.buyer_spec = buyer_spec.capitalize()

    async def _get_rating_range(self, boost_type):
        ...
        try:
            ...
            if boost_type == 'Set rating' and rating_format_check:
                ...
                await self._get_rating_range(boost_type)

        except ValueError:
            await self.author.send("Rating format not recognised, please check your format and try again")
            await self._get_rating_range(boost_type)

    async def _get_price(self, boost_type, price_recommendation):
        ...
        try:
            ...
            await self._get_price(boost_type, price_recommendation)
    ...
    async def get_gold_realms(self):
        ...
        for realm in gold_realms_list:
            ...
            for x in dictionaries.connected_realms:
                ...
                if realm.capitalize() in dictionaries.realm_abbreviations.keys():
                    realm = dictionaries.realm_abbreviations[realm.capitalize()]
